...
public class ChangePasswordController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            {
                valid = false;
                error.setNewPassword("Password is invalid");
            }
            if(!newPassword.equals(confirmPassword))
            {
                error.setConfirmPassword("Confirm password not matches password");
                valid = false;
            }
            AccountDAO dao = new AccountDAO();
            AccountDTO account = dao.get(username);
            if(!oldPassword.matches(account.getPassword()))
            {
                error.setPassword("Password wrong ");
                valid = false;
            }
            if(!valid)
            {
