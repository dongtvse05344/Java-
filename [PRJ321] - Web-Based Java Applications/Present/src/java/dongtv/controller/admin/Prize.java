...
public class Prize extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url =PRIZE_VIEW;
        HttpSession session = request.getSession();
        try {
            session.setAttribute("TOOL", "Prize");
            DongtvBean bean = new DongtvBean();
            List<PrizeDTO> list = bean.getPrizes();
            for (PrizeDTO prizeDTO : list) {
                prizeDTO.setDescription(prizeDTO.getDescription().substring(0,Math.min(100,prizeDTO.getDescription().length())));
            }
            request.setAttribute("LIST", list);
        } catch (Exception e) {
            log("ERROR at Prize " +e.getMessage());
        }
        finally
        {
