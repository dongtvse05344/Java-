...
public class RegisterEvent extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if( eventId != null) session.setAttribute("EventId", eventId);
            String username = (String) session.getAttribute("USERNAME");
            if (username == null) 
            {
                url = LOGIN;
                request.setAttribute("ACTION", "LoginToRegisEvent");
            }
            else
            {
                int EventId = Integer.parseInt((String)session.getAttribute("EventId"));
                DongtvBean bean = new DongtvBean();
                bean.setRankDTO(new RankDTO(username, EventId, 0,0));
                bean.createRank();
            }
        } catch (Exception e) {
            log("ERROR at Register Event " + e.getMessage());
        } finally {
